...
public class SquadSummaryReport implements Report {
    ...
    private Long getMin(Role r, Tactic t, Iterable<Player> ps) {
        return Math.round((double) Player.byRating(r, t).min(ps).getRating(r, t) / 100);
    }
    ...
    public void print(PrintWriter w) {
        ...
        w.format("%28s ", "Max");
        for (Role r : roles) {
            w.format("%5s ", Math.round((double) Player.byRating(r, Tactic.NORMAL).max(squad.players()).getRating(r, Tactic.NORMAL) / 100));
        }
        w.println();
