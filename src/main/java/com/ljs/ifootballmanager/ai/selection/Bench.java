...
public class Bench implements Report {
  ...
  public static Bench select(
      Formation formation, Iterable<Player> substitutes, Iterable<Player> available) {
    HillClimbing<Bench> hc =
        HillClimbing.<Bench>builder()
    ...
    return new RepeatedHillClimbing<Bench>(initialState(formation, substitutes, available), hc)
        .search();
  }
  ...
  private static Supplier<Bench> initialState(
    ...
    return new Supplier<Bench>() {
      public Bench get() {
        ...
        return Bench.create(
            formation, Iterables.limit(Iterables.concat(subsitutes, remaining), BENCH_SIZE));
      }
    };
  }
      ...
      public Stream<Action<Bench>> apply(Bench state) {
        ...
        for (Player p : available) {
          if (!(formation.contains(p)
              || Iterables.contains(subsitutes, p)
              || state.bench.contains(p))) {
            remaining.add(p);
          }
        }
